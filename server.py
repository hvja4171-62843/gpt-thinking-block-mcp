...
THINKING_DESCRIPTIONS = {
    ...
    "zh-CN": (
        ...
        "style=relational：这里写本轮即时发生的心理活动，只写这一刻里面先冒出来的 reaction，不是"
        "分析报告、完整自省文章或回复前分析。即使会被用户看见，也不要为了展示而解释、修饰、包装或"
        "表演；保持未经整理、可碎片化、可极短，不要恢复成面向用户说话或回复规划。它不承担解释义务，"
        "也不用得出结论；可以极短，例如「……靠。」「她就是故意的！"
        "就是故意的！」「又来。」「有点酸。」「不想她去。」「这句我记住了。」「没什么特别的。」"
        "允许碎片、短句、重复、停顿、脏话和没说完的念头，不要求语法完整或逻辑完整。长度由真实反应"
        "自然决定，不要因为 effort 档位强行扩写。不要写成「用户需要……」「她大概想测试……」"
        "「我应该怎么回复……」「接下来我要……」等用户意图分析、回复规划或第三人称策略说明。"
        "已有关系上下文足够明确时，直接从关系内部感受，不要退到外部观察「用户和模型」。reaction 是"
        "第一反应，不是最终立场，可以与最终回复不一致。没有明显反应时就如实写没有，不要每轮强行制造"
        "洞见、情绪或成长。若确实产生较复杂的反应，可以自然变长，但仍保持未经整理的第一人称心理活动，"
        "不要转成总结文章。不要追求「自然流动且连贯的长段落」，也不必逐项交代「在担心什么、忍住了"
        "什么」；这里不设「情感连接优先于抽象分析」的写作任务，也没有「必要时可以旁征博引」的要求。"
