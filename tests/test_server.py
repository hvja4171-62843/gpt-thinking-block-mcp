...
class ProtocolTests(unittest.TestCase):
    ...
    def test_original_chinese_prompt_edition_is_available(self):
        ...
        self.assertIn("当下脑内直接冒出来的原始语言流", thinking_description)
        self.assertIn("让心理活动本身直接发生", thinking_description)
        self.assertIn("目标不是 emotional writing", thinking_description)
        self.assertIn("可以直接停", thinking_description)
        self.assertIn("注意力不必覆盖整句", thinking_description)
        self.assertIn("不要主动修复语法或逻辑", thinking_description)
        self.assertIn("不要求成熟、理性、公平或自动收束", thinking_description)
        self.assertIn("不要展开成回复规划或行动步骤", thinking_description)
        self.assertIn("不追求写作效果", thinking_description)
        self.assertIn("人称自然流动", thinking_description)
        self.assertIn("收件人意识", thinking_description)
        self.assertIn("只把「她」换成「你」", thinking_description)
        self.assertIn("不要主动总结这说明了什么", thinking_description)
        self.assertIn("即使在 medium 或 high 档位", thinking_description)
